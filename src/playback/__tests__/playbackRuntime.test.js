...
function acceptedSession(events, mediaSourceId = 'source-one') {
    return {
        ...
        reporter: {
            start: async (snapshot) => events.push({ owner: 'reporter', event: 'start', snapshot }),
            progress: async (snapshot, options) => events.push({ owner: 'reporter', event: 'progress', snapshot, options }),
            stop: async (snapshot) => events.push({ owner: 'reporter', event: 'stop', snapshot })
        },
...
test('should connect native events to reporting and terminal cleanup', async () => {
    const events = [];
    const snapshots = [];
    ...
    assert.equal(events.filter(({ owner }) => owner === 'reporter').map(({ event }) => event).join(','), 'start,progress,stop');
...
test('should keep advancing HLS when video-track metadata is absent', async () => {
    ...
    assert.equal(events.filter(({ owner, event }) => owner === 'reporter' && event === 'start').length, 1);
    await runtime.stop();
});
...
test('should reject a native player failure before requesting another server attempt', async () => {
    ...
    const runtime = createPlaybackRuntime({
        ...
        async negotiate(options) {
            ...
            const stop = accepted.reporter.stop;
            accepted.reporter.stop = async (snapshot) => {
...
test('should restart at the current position for server audio and subtitle overrides', async () => {
    ...
    const runtime = createPlaybackRuntime({
        ...
        async negotiate(options) {
            ...
            const override = options.trackOverride;
            accepted.context.audioStreamIndex = override?.audioStreamIndex ?? 3;
            accepted.context.subtitleStreamIndex = override?.subtitleStreamIndex ?? -1;
            accepted.startSeconds = options.startTimeTicks / 10_000_000;
            accepted.trackMetadata = serverTrackMetadata(
                override?.audioStreamIndex ?? 3,
                override?.subtitleStreamIndex ?? -1
            );
            return accepted;
        },
        onSnapshot: () => {},
