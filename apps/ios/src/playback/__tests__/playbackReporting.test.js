...
import { createPlaybackReporter } from '../playbackReporter.js';
...
test('should report start progress and stop once in order', async () => {
    ...
    let time = 0;
    const reporter = createPlaybackReporter({ client, context: context(), now: () => time });

    await reporter.start({ positionTicks: 10, isPaused: false });
    time = 11_000;
    await reporter.progress({ positionTicks: 20, isPaused: false });
    await reporter.stop({ positionTicks: 30, isPaused: true });
    await reporter.stop({ positionTicks: 40, isPaused: true });
...
test('should throttle routine progress but allow forced pause reports', async () => {
    ...
    await reporter.start({ positionTicks: 0, isPaused: false });
    time = 1_000;
    assert.equal(await reporter.progress({ positionTicks: 10, isPaused: false }), false);
    assert.equal(await reporter.progress({ positionTicks: 10, isPaused: true }, { force: true }), true);
...
test('should attach source track and correlation fields to reports', async () => {
    ...
    await reporter.start({ positionTicks: 123, isPaused: false });
...
test('should keep playback running when a report request fails', async () => {
    const events = [];
    const reporter = createPlaybackReporter({
    ...
    assert.equal(await reporter.start({ positionTicks: 0, isPaused: false }), false);
