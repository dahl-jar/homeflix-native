...
function sessionPayload(context, snapshot) {
    return {
...
export function createPlaybackSessionMonitor({
    client,
    context,
    telemetry = null,
    now = Date.now,
    ...
    return {
        async start(snapshot) {
            ...
            const sent = await send('playback_start', reportPlaybackStart, sessionPayload(context, snapshot));
            if (sent) {
                started = true;
                lastProgressAt = now();
            }
        ...
        async progress(snapshot, { force = false } = {}) {
            ...
            const sent = await send('playback_progress', reportPlaybackProgress, sessionPayload(context, snapshot));
        ...
        async stop(snapshot) {
            ...
            return send('playback_stop', reportPlaybackStop, {
                ...sessionPayload(context, snapshot),
